...
import com.app.utils.model.UException;
import com.app.utils.model.UExceptionType;
...
public class UriParse {
    ...
    public static String parseOwnUri(Context context, Uri uri) {
        ...
        if (TextUtils.equals(uri.getAuthority(), Constant.getFileProviderName(context))) {
            path = new File(uri.getPath().replace("common_utils/", "")).getAbsolutePath();
        } else {
            path = uri.getPath();
        }
        return path;
     ...
     * @return
     */
    public static String getFilePathWithUri(Uri uri, Activity activity) throws UException {
        if (uri == null) {
            Log.w(TAG, "uri is null,activity may have been recovered?");
            throw new UException(UExceptionType.TYPE_URI_NULL);
        }
        File picture = getFileWithUri(uri, activity);
        String picturePath = picture == null ? null : picture.getPath();
        if (TextUtils.isEmpty(picturePath))
            throw new UException(UExceptionType.TYPE_URI_PARSE_FAIL);
        return picturePath;
    }
     ...
     * @return
     */
    public static String getImgPathWithUri(Uri uri, Activity activity) throws UException {
        String picturePath = getFilePathWithUri(uri, activity);
        if (!UImageFiles.checkMimeType(activity, UImageFiles.getMimeType(activity, uri)))
            throw new UException(UExceptionType.TYPE_NOT_IMAGE);
        return picturePath;
    }
     ...
     * @return
     */
    public static String getFilePathWithDocumentsUri(Uri uri, Activity activity) throws UException {
        if (uri == null) {
        ...
        if (ContentResolver.SCHEME_CONTENT.equals(uri.getScheme()) && uri.getPath().contains("document")) {
            File tempFile = UImageFiles.getTempFile(activity, uri);
            try {
                UImageFiles.inputStreamToFile(activity.getContentResolver().openInputStream(uri), tempFile);
                return tempFile.getPath();
            } catch (FileNotFoundException e) {
                e.printStackTrace();
                throw new UException(UExceptionType.TYPE_NO_FIND);
            }
        } else {
